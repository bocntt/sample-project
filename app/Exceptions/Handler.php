    ...
    public function render($request, Exception $exception)
    {
        ...
        switch ($exception) {
            ...
            case $exception instanceof UnauthorizedException:
                if($request->ajax()) {
                    return response()->json(['error' => 'Unautorized'], 500);
                }
                return response()
                        ->view('errors.unauthorized-exception', compact('exception'), 500);
                break;
            case $exception instanceof NoActiveAccountException:
                ...
                return response()
                        ->view('errors.no-active-account-exception', compact('exception'), 500);
                break;
